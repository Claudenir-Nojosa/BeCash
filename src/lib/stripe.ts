...
export async function setupStripeProducts() {
  try {
    ...
    console.log('STRIPE_FAMILY_MONTHLY_USD_PRICE_ID=', familyMonthlyPriceUSD.id);
    ...
    return {
      ...
      familyPrices: {
        brl: { monthly: familyMonthlyPriceBRL.id, yearly: familyYearlyPriceBRL.id },
        usd: { monthly: familyMonthlyPriceUSD.id, yearly: familyYearlyPriceUSD.id },
      },
    };
  } catch (error) {
...
export async function getPriceId(params: {
  ...
  const priceMap: Record<string, Record<string, Record<string, string>>> = {
    basic: {
      BRL: { monthly: process.env.STRIPE_BASIC_PRICE_ID!, yearly: process.env.STRIPE_BASIC_PRICE_ID! },
      USD: { monthly: process.env.STRIPE_BASIC_PRICE_ID!, yearly: process.env.STRIPE_BASIC_PRICE_ID! },
    },
    pro: {
      BRL: {
